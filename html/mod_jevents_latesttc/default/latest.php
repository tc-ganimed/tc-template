...
class DefaultModLatestView
{
	...
	function displayLatestEvents()
	{
		...
		if (isset($this->eventsByRelDay) && count($this->eventsByRelDay))
		{
			$content .= '<img src="/images/website/cat_archive.jpg" alt="" />';
			$content .= '<ul class="mod_events_latest_table">';
			...
			foreach ($this->eventsByRelDay as $relDay => $daysEvents)
			{
				...
				$content .= '<li class="mod_events_latesttc_row">';
				foreach ($daysEvents as $dayEvent)
				{

					//$content .= '<div class="mod_events_latesttc_cell">';
		...
		else
		{
			...
			$content .= '<div class="mod_events_latest_row'.$k.'"><div class="mod_events_latest_cell">' . JText::_('JEV_NO_EVENTS') . '</div></div>' . "\n";
			$content .="</div>\n";
		}
